...
site/                            → optional Astro UI for digests (lives in .infra/site)
```
...
| `.infra/` | Scripts, Templates, Astro site (hidden in Obsidian) |
...
cd .infra/site && npm install && npm run dev
```
...
- Daily Note template: `.infra/Templates/Daily Research Link.md`
- At **08:15** local, a LaunchAgent opens today’s digest in Obsidian (`.infra/scripts/open-in-obsidian.sh`)
